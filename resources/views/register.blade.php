    ...
    <div class="container mt-5 mb-5">
        <form method="POST" action="/register_user">
            ...
            <div class="form-row align-items-center">
              ...
              <h5 class="error">@error('imie') {{$message}}<br> @enderror @error('nazwisko') {{$message}} <br>@enderror @error('email') {{$message}} <br>@enderror @error('password') {{$message}} <br>@enderror @error('haslo2') {{$message}} <br>@enderror @if(Session::has('fail')) {{Session::get('fail')}}  @endif</h5>
              <br>

              <div class="col-auto my-1 ms-auto me-auto text-center">
              </div>
              <div class="col-auto my-1 ms-auto me-auto text-center">
                <button type="submit" class="btn signin_button">Zarejestruj się</button>
              </div>
            </div>
